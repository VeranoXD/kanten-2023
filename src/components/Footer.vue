<template>
  <footer class="shadow  bg-black">
    <div class="w-full max-w-screen-xl mx-auto p-4 md:py-8">
      <div class="sm:flex sm:items-center sm:justify-between">
        <a href="" class="flex items-center mb-4 sm:mb-0">
          <img class="logo w-16" src="../assets/img/fanlogo.jpg" alt="Kanten Logo" />
        </a>
        <ul
          class="flex flex-wrap items-center mb-6 text-sm font-medium text-gray-500 sm:mb-0 dark:text-gray-400"
        >
          <li>
            <a href="https://www.facebook.com/kanten.esbjerg/" class="mr-4 hover:underline md:mr-6">Facebook</a>
          </li>
          <li>
            <a href="https://www.instagram.com/husetesbjerg/" class="mr-4 hover:underline md:mr-6">Instagram</a>
          </li>
          <li>
            <a href="https://www.linkedin.com/company/huset-esbjerg/" class="mr-4 hover:underline md:mr-6">LinkedIn</a>
          </li>
        </ul>
      </div>
      <hr
        class="my-6 border-gray-500 sm:mx-auto dark:border-gray-700 lg:my-8"
      />
      <div class="spacer flex justify-between sm:mx-auto">
        <div class="info flex space-x-1 sm:mx-auto">
            <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-map-pin text-gray-500" width="20" height="20" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
   <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
   <path d="M9 11a3 3 0 1 0 6 0a3 3 0 0 0 -6 0"></path>
   <path d="M17.657 16.657l-4.243 4.243a2 2 0 0 1 -2.827 0l-4.244 -4.243a8 8 0 1 1 11.314 0z"></path>
</svg>
        <h1
          class="block text-sm text-gray-500 sm:text-center dark:text-gray-400 "
        >
          Gl. Novrupvej 14, 6705 Esbjerg Ø</h1
        >
    </div>

        <div class="info flex space-x-1 sm:mx-auto">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            class="icon icon-tabler icon-tabler-phone text-gray-500"
            width="20"
            height="20"
            viewBox="0 0 24 24"
            stroke-width="2"
            stroke="currentColor"
            fill="none"
            stroke-linecap="round"
            stroke-linejoin="round"
          >
            <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
            <path
              d="M5 4h4l2 5l-2.5 1.5a11 11 0 0 0 5 5l1.5 -2.5l5 2v4a2 2 0 0 1 -2 2a16 16 0 0 1 -15 -15a2 2 0 0 1 2 -2"
            ></path>
          </svg>
          <h1
            class="block text-sm text-gray-500 sm:text-center dark:text-gray-400"
            >28 97 11 85</h1
          >
        </div>

        <div class="info flex space-x-1 sm:mx-auto">
        <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-mail text-gray-500" width="20" height="20" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
   <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
   <path d="M3 7a2 2 0 0 1 2 -2h14a2 2 0 0 1 2 2v10a2 2 0 0 1 -2 2h-14a2 2 0 0 1 -2 -2v-10z"></path>
   <path d="M3 7l9 6l9 -6"></path>
</svg>
        <h1
          class="block text-sm text-gray-500 sm:text-center dark:text-gray-400 "
        >
        Musikforeningenkanten@gmail.com</h1
        >
        </div>
      </div>
    </div>
  </footer>
</template>

<script setup>
</script>

<style scoped>

@media (max-width: 640px) {
  .spacer{
    flex-direction: column;
  }
  .logo {
    display: none;
  }
}
</style>